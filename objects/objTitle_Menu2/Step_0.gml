...
if(objTitle_Escape2.y < 122 && escape_start){
	objTitle_Escape2.y += (escape_target - objTitle_Escape2.y) / escape_speed;
}
if(objTitle_Escape2.y > 110){
	objTitle_Todd2.settle_in = true;
	escape_fade = true;
}else{
	objTitle_Escape2.image_alpha = 1 - (objTitle_Escape2.y / (escape_target - 2));
}

if(escape_fade){
	if(escape_fadein){
		objTitle_Escape2.image_alpha += 0.01;
		if(objTitle_Escape2.image_alpha >= 1) escape_fadein = false;
	}else{
		objTitle_Escape2.image_alpha -= 0.01;
		if(objTitle_Escape2.image_alpha <= 0) escape_fadein = true;
	}
}